...
async function setRole(userId, role) {
    const action = role === 'admin' ? 'make this user an admin' : 'remove admin privileges from this user';
    if (!confirm(`Are you sure you want to ${action}?`)) return;

    const endpoint = role === 'admin' ? 'set_role_to_admin.php' : 'set_role_to_user.php';
    ...
    try {
        ...
        if (data.success) {
            ...
            roleButton.textContent = role === 'admin' ? 'Make User' : 'Make Admin';
            roleButton.onclick = () => setRole(userId, role === 'admin' ? 'user' : 'admin');
