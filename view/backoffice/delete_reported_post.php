...
try {
    ...
    // Check if post exists and is reported
    $stmt = $db->prepare("SELECT f.id FROM forum_posts f 
                         INNER JOIN post_reports r ON f.id = r.post_id 
                         WHERE f.id = ?");
    $stmt->execute([$post_id]);
    if (!$stmt->fetch()) {
        throw new Exception('Reported post not found');
    }
