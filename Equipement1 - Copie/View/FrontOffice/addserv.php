...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($service == 1) {
        ...
        if (empty($errors)) {
            $result = addService($pdo, $title, $solarNumber, $price, $service, $type,$id_user);
            if ($result) {
        ...
        if (empty($errors)) {
            ...
            $result = addService($pdo, $title, $total, $price, $service, $type,$id_user);
            if ($result) {
                header('Location: home.php'); 
                exit();
            } else {
        ...
        if (empty($errors)) {
            $result = addService($pdo, $title, $personne, $price, $service, $type,$id_user);
            if ($result) {
                header('Location: home.php');
                exit();
            } else {
        ...
        if (empty($errors)) {
            $result = addService($pdo, $title, $chambres, $price, $service, $type,$id_user);
            if ($result) {
                header('Location: home.php');
                exit();
            } else {
        ...
        if (empty($errors)) {
            $result = addService($pdo, $title, $jardin, $price, $service, $type,$id_user);
            if ($result) {
                header('Location: home.php');
                exit();
            } else {
