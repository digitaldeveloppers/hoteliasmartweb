...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_message_id'], $_POST['reply_content'])) {
    ...
    if ($reply_message_id && $reply_content !== '') {
        $msg_sql = "SELECT sender_id FROM support_messages WHERE message_id = $reply_message_id";
        ...
        if ($msg_result && $msg_result->num_rows > 0) {
            ...
            $stmt->bind_param('iis', $sender_id, $receiver_id, $reply_content);
            if ($stmt->execute()) {
                $reply_sent = true;
            } else {
...
<head>
    ...
    <title>Admin Messages - Hotelia Smart Admin Dashboard</title>
