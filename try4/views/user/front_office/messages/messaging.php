...
// Fetch all admins
$admins = [];
$admin_sql = "SELECT user_id, first_name, last_name, email FROM user WHERE role = 'admin'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['receiver_id'], $_POST['message_content'])) {
    ...
    if ($receiver_id && $message_content !== '') {
        $stmt = $conn->prepare("INSERT INTO support_messages (sender_id, receiver_id, message_content) VALUES (?, ?, ?)");
...
$messages_sql = "SELECT m.*, u1.first_name AS sender_first, u1.last_name AS sender_last, u2.first_name AS receiver_first, u2.last_name AS receiver_last FROM support_messages m JOIN user u1 ON m.sender_id = u1.user_id JOIN user u2 ON m.receiver_id = u2.user_id WHERE m.sender_id = $user_id OR m.receiver_id = $user_id ORDER BY m.created_at DESC";
...
<body style="background: #f5f7fa;">
    <div class="msg-container">
        <h2 style="text-align:center; font-weight:700; color:#2d3e50; margin-bottom:24px;">Contact Admin Support</h2>
        <?php if ($message_sent): ?>
        ...
        <form method="POST" class="msg-form">
            <div class="form-group">
                <label for="receiver_id">Send to Admin:</label>
